...
const FooterLinks = ({ title, data, showList, className,slug }) => {
  return (
    <div className={`footer-links mb-[30px] ${className}`}>
      {title && (
        <h4 className="relative mb-2 inline-block pb-0.5 pr-3 text-base font-semibold uppercase before:absolute before:bottom-0 before:left-0 before:right-0 before:h-[1px] before:bg-green before:content-['']">
          {title}
        </h4>
      )}
      <ul>
        {data?.slice(0, showList)?.map((item, index) => (
          <li className="mb-3 uppercase text-[#eee]" key={index}>
            <Link
